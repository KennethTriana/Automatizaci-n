#language: es
  #author: Nicol Sanchez

Característica: Editar una categoria en el Sistema de Inventario LGC
  Como usuario del Sistema de Inventario LGC
  Quiero editar una categoria en el Sistema de Inventario LGC


  Antecedentes:
    Dado que el usuario se encuentra en la pagina de inicio de sesion del aplicativo.
    Cuando ingrese las credenciales corretamente (correo y contrasena).
      |email|contrasena|
      |nelson@gmail.com|123|

  @categoria2
  Escenario: Editar una categoria en el Sistema de Inventario LGC
    Cuando ingrese credenciales correctas para editar la categoria.

      |Carnes       |

    Entonces se debe verificar que el usuario haya editado la categoria en el Sistema de Inventario LGC.